...
if(isset($_POST["action"])){
    if($_POST["action"]=="fetch"){
        ...
        if(count($folder)>0){
            foreach($folder as $name){
                $output .='
                    <tr>
                        ...
                        <td><button type="button" name="view_files" data-name="'.$name.'" class="view_files btn btn-secondary">Ver archivos</button></td>
                        <td><button type="button" name="delete" data-name="'.$name.'" class="delete view_files btn btn-danger">Borrar carpeta</button></td>
                    </tr>
                ';
            }
        }else{
            $output .='
                <tr>
                    <td colspan="6">No hay ninguna carpeta</td>
                </tr>
            ';
        }
        $output .='</table></div>';
    ...
    if($_POST["action"]=="fetch_files"){
        ...
        <table class="table table-bordered table-striped" width="800px">
            <tr>
                ...
                <th>Tamaño en bytes del archivo</th>
        ...
        foreach($file_data as $file){
            if($file === '.' OR $file ==='..'){
                ...
                $output .='
                    <tr>
                        <td>'.$file.'</td>
                        <td><a type="button" href="editor.php?openid='.$path.'" name="open_file" id="open_file" class="btn btn-primary">Abrir</a></td>
